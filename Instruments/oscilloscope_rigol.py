...
averages as well as query the current sample rate of the oscilloscope."""
    ...
    class Channel:
        ...
        def set_bandwidth_limit(self, channel, bw):
            ...
            if channel in allowed_chnl_values and bw in allowed_type_values:
                comm = f":CHANnel{channel}:BWLimit {bw}"
        ...
        def get_bandwidth_limit(self, channel):
            ...
            comm_mode = f":CHANnel{channel}:BWLimit?"
            return self.instrument.query(comm_mode)

        def set_coupling_mode(self, channel, coupling_mode):
            """Set the coupling mode of the specified channel."""
            allowed_chnl_values = [1, 2]
            allowed_type_values = ["AC", "DC", "GND"]
            if channel in allowed_chnl_values and coupling_mode in allowed_type_values:
                comm = f":CHANnel{channel}:COUPling {coupling_mode}"
                self.instrument.write(comm)
            else:
        ...
        def get_coupling_mode(self, channel):
            ...
            comm_mode = f":CHANnel{channel}:COUPling?"
            return self.instrument.query(comm_mode)
        ...
        def invert_waveform(self, channel, param1):
            ...
            if channel in allowed_chnl_values and param1 in allowed_type_values:
                comm = f":CHANnel{channel}:INVert {param1}"
                self.instrument.write(comm)
            else:
        ...
        def is_inverted(self, channel):
            ...
            comm_mode = f":CHANnel{channel}:INVert?"
            return self.instrument.query(comm_mode)

        def set_offset(self, channel, param1):
            """Set the vertical offset of the specified channel. The default unit is V."""
            allowed_chnl_values = [1, 2]
            if channel in allowed_chnl_values:
                comm = f":CHANnel{channel}:OFFSet {param1}"
                self.instrument.write(comm)
            else:
        ...
        def get_offset(self, channel):
            ...
            comm_mode = f":CHANnel{channel}:OFFSet?"
            return self.instrument.query(comm_mode)

        def set_range(self, channel, param1):
            """Set the vertical range of the specified channel. The default unit is V."""
            allowed_chnl_values = [1, 2]
            if channel in allowed_chnl_values:
                comm = f":CHANnel{channel}:RANGe {param1}"
                self.instrument.write(comm)
            else:
                print("Invalid channel.")
        ...
        def get_range(self, channel):
            ...
            comm_mode = f":CHANnel{channel}:RANGe?"
            return self.instrument.query(comm_mode)

        def set_tcal(self, channel, val):
            """
    ...
    class Decoder:
        ...
        def set_threshold_channel(self, channel, threshold):
            """
            ...
            if channel not in [1, 2]:
                ...
                return
            self.instrument.write(f":DECoder{self.n}:THREshold:CHANnel{channel} {threshold}")
        ...
        def get_threshold_channel(self, channel):
            ...
            if channel not in [1, 2]:
                print("Invalid channel. Use 1 or 2.")
                return None
            return float(self.instrument.query(f":DECoder{self.n}:THREshold:CHANnel{channel}?"))
        ...
        def get_parallel(self):
            ...
            None
            Return:
            Oscilloscope.Decoder.Parallel: Parallel decoder object
            """
            return Oscilloscope.Decoder.Parallel(self.instrument, self.n)
