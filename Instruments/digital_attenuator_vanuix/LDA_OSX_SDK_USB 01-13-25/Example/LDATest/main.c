...
//  LDATest
//
//  Created by Robert Dezmelyk on 2/11/19.
//  Copyright © 2019-2025 Vaunix Technology Corp.
//
//  RD 1/11/25 updated to show use of more HR functions where attenuation values are specified in .05 db units for the API
//
...
static float AStop = 63.0;          // default atten stop, for a lot of devices this is 63 db so we use that.
...
static float AStep = 0.5;           // default step size is .5db, some LDA's have larger minimum steps
static float AStep2 = 0.5;          // default second phase step size for LDA's that support bidirectional ramps
...
void PrintHelp(void)
{
    ...
    printf("  -m        Test mode, simulates the basic operation two LDA devices");
    printf("  -d i n     Select the devices to work with, i can be either the device number (1,2,3, etc.)\n");
    printf("             or device serial number, and n is the number of devices to apply the command to.\n");
    ...
    printf("  -f nn     Set working frequency, nn is working frequency in MHz (e.g. 2450.1)\n");
    printf("  -a nn     Set attenuation, nn is attenuation in db (e.g. 25.5)\n");
    printf("  -w nn     Set idle time between attenuator ramps, nn is time in ms.\n");
    printf("  -h nn     Set hold time between ramp phases\n");
    printf("  -s nn     Set ramp start value, nn is start value in db\n");
    printf("  -e nn     Set ramp end value, nn is end value in db, p is ramp phase\n");
...
char* fnLDA_perror(LVSTATUS status) {
    ...
    if (status == FEATURE_NOT_SUPPORTED) strcpy(errmsg, "FEATURE_NOT_SUPPORTED");
...
void CheckAPISet(LVSTATUS status)
{
    ...
    {
        printf("*** Error: LDA API returned status = %x, %s ***\n", status, fnLDA_perror(status));
    }
    
}

/* A function to display the status as an ASCII string */
char* fnLDA_pstatus(LVSTATUS status) {
    ...
    if (status == SWP_ACTIVE) strcpy(errmsg, "SWP_ACTIVE");
    if (status == SWP_UP) strcpy(errmsg, "SWP_UP");
    if (status == SWP_REPEAT) strcpy(errmsg, "SWP_REPEAT");
    if (status == SWP_BIDIRECTIONAL) strcpy(errmsg, "SWP_BIDIRECTIONAL");
    if (status == PROFILE_ACTIVE) strcpy(errmsg, "PROFILE_ACTIVE");
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (state == wantDash)
        {
            ...
            switch (state){
                ...
                case wantIdle:
                    IdleTime = atoi(thisParam);   // convert to an int
                    state = wantDash;
                    break;
                ...
                case wantWorkingFrequency:
                    WorkingFrequency = (float) atof(thisParam);    // cast to a float, atof actually returns a double
                    state = wantDash;
                    break;
...
int main(int argc, const char * argv[])
{
    int i, j, k;
    int iDev;
    int itemp = 0;              // init to clear a warning only
    bool bTemp;
    float ftemp;
    int NumChannels = 1;        // temporary storage for the number of channels in a device
    ...
    fnLDA_SetTestMode(gbWantSetTestMode);
    
    // Fill MyDevices array with connected devices
    ...
    if (DevNum > MAXDEVICES)
    {
        for(j = 0; j < i; ++j)
        {
            k = fnLDA_GetSerialNumber(MyDevices[j]);
            if (k == DevNum)
            {
                DevNum = j;
                break;
    ...
    {
        // set range to display info for all connected devices if user does not specify a device
        if(!gbWantSetDev)
        {
            DevRange = NumDevices;
        }
        for (j = DevNum; j < DevNum + DevRange; j++){
        ...
        for (iDev = DevNum; iDev < DevNum + DevRange; iDev++)
        {
            ...
            // --- Lets set the attenuation for the channel (or device) first ---
            ...
            if (gbWantSetAStart)
            {
                ...
                itemp = fnLDA_SetRampStartHR(MyDevices[iDev], itemp);
                CheckAPISet(itemp);
            }
            ...
            if (gbWantSetAStop)
            {
                ...
                itemp = fnLDA_SetRampEndHR(MyDevices[iDev], itemp);
                CheckAPISet(itemp);
            }
            ...
            if (gbWantSetAStep)
            {
                ...
                itemp = fnLDA_SetAttenuationStepHR(MyDevices[iDev], itemp);
                CheckAPISet(itemp);
            }
            ...
            if (fnLDA_GetFeatures(MyDevices[iDev]) > 0)
            {
                if (gbWantSetAStep2)
                {
                    ...
                    itemp = fnLDA_SetAttenuationStepTwoHR(MyDevices[iDev], itemp);
                    CheckAPISet(itemp);
                }
                if (gbWantSetDwell2)
                {
        ...
        if (!gbBatchMode)
        {
            ...
            while (j < 10)
            {
                for (iDev = DevNum; iDev < DevNum + DevRange; iDev++)
                {
                    ...
                    // use the HiRes function and show all eight channels
